...
def route_query(
    ...
    if mode_key == "benchmark":
        return run_benchmark(query, source, progress_callback=progress_callback)
    if mode_key in {"approx", "fast", "balanced", "accurate"}:
        approx_mode = "balanced" if mode_key == "approx" else mode_key
        return run_approx(query, source, mode=approx_mode, progress_callback=progress_callback)
